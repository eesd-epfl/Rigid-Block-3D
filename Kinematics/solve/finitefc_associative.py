...
from .util import _update_elem_disp_2d, _line1, _line2, _line3, _line4, _update_contp_force_2d,_update_elem_disp_3d
import numpy as np
print_detail = False


def solve_finitefc_associative(elems, contps, Aglobal=None,BC = 'cantilever'):
    ...
    """
    if get_dimension() == 2:
        solution = solve_finitefc_associative_2d(
        ...
        return solve_finitefc_associative_3d(elems, contps,Aglobal=Aglobal,BC=BC)


def solve_finitefc_associative_3d(elems, contps,Aglobal=None,BC = 'cantilever'):
    ...
    with mosek.Env() as env:
        ...
        with env.Task(0, 0) as task:
            ...
            for element in elems.values():
                ...
                # elif element.type == "mortar":
                #     maximum_mortar_moment_f = (element.mass)**2*1e6*6.1*0.33*10*1/1.41*2*5/6/1000
                #     maximum_mortar_moment_s = element.mass*1e3*(0.1+6.1*0.4*0.1)*2*10**2*0.5/1000
                #     maximum_mortar_moment = min(maximum_mortar_moment_f, maximum_mortar_moment_s)
                #     maximum_mortar_moment = maximum_mortar_moment_f
                #     maximum_mortar_force = element.mass*1e3*0.05*2*10*1/1.41*2/1000
                #     bkc.extend([mosek.boundkey.ra,
                #                 mosek.boundkey.ra,
                #                 mosek.boundkey.ra,
                #                 mosek.boundkey.ra,
                #                 mosek.boundkey.ra,
                #                 mosek.boundkey.ra])
                #     blc.extend([-maximum_mortar_force,
                #                 -maximum_mortar_force,-maximum_mortar_force, -maximum_mortar_moment, -maximum_mortar_moment, -maximum_mortar_moment])
                #     buc.extend([maximum_mortar_force,
                #                 maximum_mortar_force, maximum_mortar_force, maximum_mortar_moment, maximum_mortar_moment, maximum_mortar_moment])
                else:
                    bkc.extend([mosek.boundkey.fx,
                                mosek.boundkey.fx,
...
def solve_finitefc_associative_2d(elems, contps, Aglobal=None):
    """Solve 2D limit force using associative flow rule considering finite compressive strength
    ...
    contfs = dict()
    for p in tqdm(contps.values(), desc='assemble contact faces'):
        if p.faceID not in contfs.keys():
            ...
            contfs[p.faceID].contps.append(p.id)
    face_key_to_index = {k: i for i, k in enumerate(contfs)}
    ...
    with mosek.Env() as env:
        ...
        with env.Task(0, 0) as task:
            ...
            blc = []
            buc = []
            for key, value in tqdm(elems.items(), desc='Bound keys and values for constraints on equilbrium'):
            ...
            for key, value in tqdm(contps.items(), desc='Bound keys and values for constraints on sliding'):
                if value.cont_type.type == 'friction' or value.cont_type.type == 'friction_fc':
                    ...
                    blc.extend([-inf, -inf, -inf])
                    #Ft = value.cont_type.ft*value.section_h
                    buc.extend(
                        [value.cont_type.cohesion, value.cont_type.cohesion, +inf])
                else:
                    raise NameError("unknown contact type!")
            # Bound keys and values for constraints -- crushing failure condition
            for key, value in tqdm(contfs.items(), desc='Bound keys and values for constraints on fc'):
            ...
            bux = []
            g_index = 0
            for key, value in tqdm(contps.items(), desc='bound key for variables'):
                for i in range(2):
                    bkx.append(mosek.boundkey.fr)
                    blx.append(-inf)
                    bux.append(+inf)
            ...
            # Objective coefficients
            c = []
            for key, value in tqdm(contps.items(), desc='Objective coefficients'):
                # for i in range(2):  # 2variables(t,n)*1nodes*2contact faces
            ...
            for i, value in tqdm(enumerate(contps.values()), desc='assemble A matrix'):
                for j in range(2):  # 2variables(t,n)*1nodes*
                    ...
                    if j == 1:
                        ...
                        line1_coeff = _line1(value.cont_type.fc, value.cont_type.ft, value.section_h)
                        line2_coeff = _line2(value.cont_type.fc, value.cont_type.ft, value.section_h)
                        line3_coeff = _line3(value.cont_type.fc, value.cont_type.ft, value.section_h)
                        line4_coeff = _line4(value.cont_type.fc, value.cont_type.ft, value.section_h)
                        # ? should points on the antagonist face be considered?
                        # Face where the point is a candidate
                        ...
                        face_index = face_key_to_index[value.faceID]
                        col_index.extend(
                            list(range(_start_row_crushing+face_index*8, _start_row_crushing+face_index*8+8)))
                        
                        col_value.extend([line1_coeff[0]+_coeff*value.lever,
                                            line1_coeff[
                           ...
                           line4_coeff[
                            0]+_coeff*value.lever,
                            line4_coeff[0]-_coeff*value.lever])

                        # Face where the point is an antagonist
                        ...
                        if point_rank == 0:
                            ...
                            raise NameError(
                                "more than two points are found for one face")
                        face_index = face_key_to_index[contps[value.counterPoint].faceID]
                        col_index.extend(
                            list(range(_start_row_crushing+face_index*8, _start_row_crushing+face_index*8+8)))
                        col_value.extend([line1_coeff[0]+_coeff*value.lever,
                                            line1_coeff[
            ...
            col_value = []
            i = 0
            for key, value in tqdm(elems.items(), desc='apply load'):
            ...
            for j in tqdm(range(numvar), desc='input var data to mosek'):
            ...
            for i in tqdm(range(numcon), desc='input con data to mosek'):
                task.putconbound(i, bkc[i], blc[i], buc[i])
            ...
            if (solsta_bas == mosek.solsta.optimal):
                ...
                # for key, value in tqdm(contps.items(), desc='check limit conditions on points'):
                ...
                # for key, value in tqdm(contfs.items(), desc='check limit conditions on faces'):
        ...
        result["contact_forces"] = xx[0:numvar-1]
        result["Lagrange_multiplier"] = y[0:numcon]
        ...
        sum = 0
        element_index = 0
        for k, value in tqdm(elems.items(), desc='normalize displacement'):
            sum += value.ll[0]*y[element_index*3]+value.ll[1] * \
                y[element_index*3+1]+value.ll[2]*y[element_index*3+2]
            element_index += 1

        max_disp = 0
        for i in tqdm(range(0, len(elems)), desc='find max disp'):
